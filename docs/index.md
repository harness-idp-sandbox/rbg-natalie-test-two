...
# run app
java -jar target/natalie-test-two-1.0-SNAPSHOT.jar
...
docker buildx build   --platform linux/amd64   -t parsontodd/natalie-test-two:latest   --push .
```
...
# watch rollout
kubectl rollout status deploy/natalie-test-two -n dev

# get external endpoint
kubectl get svc natalie-test-two-svc -n dev
```
...
```bash
kubectl port-forward svc/natalie-test-two-svc 8080:80 -n dev
...
kubectl rollout restart deploy/natalie-test-two -n dev
kubectl rollout status deploy/natalie-test-two -n dev
```
...
├─ src/
│  ├─ main/
│  │  ├─ java/com/harness/natalietesttwo/
│  │  │  ├─ App.java
...
│  │        └─ index.html
│  └─ test/
│     └─ java/com/harness/natalietesttwo/AppTest.java
├─ kubernetes/
│  ├─ deployment.yml
│  └─ service.yml
├─ Dockerfile
...
© natalie.finnegan@harness.io. For demo/PoV purposes.
